...
func DrawPoster(s Style) (fileName string, err error) {
	...
	s.OutputDIR = s.OutputDIR + "poster/"

	if s.QRCodeURL == "" || s.Title == "" || s.Content == "" {
	...
	// 获取画布，并涂白
	if s.ImageURL == "" {
		designHeight = DesignHeight - ImageHeight
	} else {
	...
	// 获取网络图片并解码
	if s.ImageURL != "" {
		fmt.Printf("s.ImageURL : %s end", s.ImageURL)
	...
	// 加载文字字体
	if font == nil {
		fontFile, err := os.Open(outputDir + "fonts/font.ttc")
		if err != nil {
...
sv:
	...
	f, err := os.OpenFile(s.OutputDIR+s.OutputFileName, os.O_CREATE|os.O_RDWR, 0755)
	if err != nil {
