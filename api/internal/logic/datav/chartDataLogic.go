...
func NewChartDataLogic(ctx context.Context, svcCtx *svc.ServiceContext) *ChartDataLogic {
...
func (l *ChartDataLogic) ChartData(req *types.ChartDataReq) (*types.ChartDataResp, error) {
	...
	if err = json.Unmarshal([]byte(resp.Dataset.GroupConfig), &groupConfig); err != nil {
		fmt.Println("---->")
		log.Fatal(err)
	}
	...
	if conf.Type == "api" {
		...
		jsonData, err = l.transformData(conf, groupConfig, params)
		if err != nil {
			//log.Fatal(err)
			return nil, err
		}
...
func (l *ChartDataLogic) transformData(apiConfig APIConfig, groupConf GroupConfig, params map[string]string) (interface{}, error) {
	...
	*/
	...
	for _, config := range apiConfig.Params {
		...
		case "date":
			...
			externalDate := l.convertDate(internalDate, config["format"])
	...
	resp, err := client.R().
		...
		Execute(apiConfig.Method, groupConf.URL+apiConfig.URL)
	if err != nil {
...
func (l *ChartDataLogic) transformObject(input map[string]interface{}, mappings []FieldMapping) (map[string]interface{}, error) {
...
func (l *ChartDataLogic) ToString(v interface{}) string {
	return fmt.Sprint(v)
}

func (l *ChartDataLogic) convertDate(internalDate, format string) string {
	parsedTime, err := l.parseTimeAttempts(internalDate)
	if err != nil {
		return ""
	}
...
func (l *ChartDataLogic) parseTimeAttempts(timeStr string) (time.Time, error) {
	// 尝试的多种时间布局
	layouts := []string{
		"2006",       // 只包含年份
		"2006-01",    // 年份和月份
